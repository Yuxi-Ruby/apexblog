...
class ArticleTest < ActiveSupport::TestCase
  ...
  test "verification of maximum title length is 40" do
  	@valid_article.title = "Title " * 8
  	@valid_article.valid?
  ...
  test "maximum description length is 150" do
  	@valid_article.description = "Description" * 16
  	puts "description length: #{@valid_article.description.length}"
  	@valid_article.valid?
  	assert_match "is too long", @valid_article.errors[:description].join, "Maximun length of :description should be validated to 150"
  end
  ...
  test "maximum body length is 1500" do
  	@valid_article.body = "Body " * 301
  	puts "body length: #{@valid_article.body.length}"
  	@valid_article.valid?
  	assert_match "is too long", @valid_article.errors[:body].join, "Maximun length of :body should be validated to 1500"
  end
